...
<a id="demo"></a>
### 效果
<img src="./static/images/fancyQrCode.jpg" width=400>  

### 案例
无限层级路由方案已经在诸多小程序中得到了实践检验，其中不乏用户体量大、交互场景复杂的案例。  
使用案例：转转二手交易网（在微信钱包中有固定入口：微信-我-支付-转转二手）、转转欢乐送、天天步数换、垃圾分类查询全能助手等
...
- 所有页面若有自定义onUnload函数，需在函数中执行父类相应钩子： super.onUnload && super.onUnload() （wepyKit中监听页面钩子功能registerPageHook目前需要页面这样手动配合，若能提供更优雅的实现，则可移除此条约束）
```js
