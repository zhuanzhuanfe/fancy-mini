...
### 小程序能力搭建/增强
- 健壮高效的登录机制
  - 使用体验：登录前后操作自然衔接、登录界面自由配置、新老用户悄悄区分、操作要求登录态时自动触发登录流程
  - 功能特性：登录态过期自动重新登录、并发调用自动合并、流程细节对业务方透明、支持多方复用、支持自定义逻辑扩展
...
- 灵活易扩展的请求管理
  - 功能：发起网络请求
  - 特点：可以方便地在请求前后添加/移除各种扩展逻辑，如：设置默认表单类型、植入cookie逻辑、植入登录态检查逻辑、植入网络异常处理逻辑等
...
- canvas工具集
  - 功能：封装了一些常用的canvas操作，提高小程序canvas易用性，如：图片居中裁剪、圆形头像、border-radius、多行文本、字符串过长截断/添加省略号等
...
- 跨页面传参
  ...
  - 方案特点：无需借用后端接口/storage，性能较好；逻辑独立、通用，对页面代码无侵入
...
- 免并发修饰器 
  - 免并发 @noConcurrent  
    ...
    示例：用户连续多次点击同一个提交按钮，只响应一次，而不是同时提交多份表单 [体验](https://zhuanzhuanfe.github.io/fancy-mini/tutorial-5.1-noConcurrent.html#demo)  
...
- 快捷兼容修饰器
  - 兼容wx格式回调 @supportWXCallback  
    ...
    场景：将回调形式的api改写为promise形式后，兼容旧代码  
